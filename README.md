...
#### Payment UI Above The Paywall (Opt-In)

Use the default `Tranzmit.of(context).presentPlacement(...)` for normal billing flows. It is the lightweight path and existing integrations do not need to change.

Use `Tranzmit.presentPlacementInRoute(...)` only when the app must show Flutter UI after the hosted paywall CTA is tapped, while the paywall is still visible. Common examples:

- Terms and conditions confirmation dialogs.
- Razorpay or custom checkout bottom sheets.
- Error dialogs or retry dialogs during payment.
- Snackbars/toasts attached to the app scaffold.
- A pushed payment or checkout screen.

The route API presents that paywall through the app's `Navigator`, so UI opened later from `onCTA` appears above the paywall instead of behind it. Paywall appearance fades in by default on both `presentPlacement()` and `presentPlacementInRoute()`. To match the host app's motion system, pass optional `transitionDuration`; omit it to use the SDK default.
...
result = Tranzmit.presentPlacementInRoute(
  ...
  onCTA: (product) async {
    ...
    final success = await startCheckout(product.id);
    if (!success) return;
...
If `onCTA` opens Flutter UI that must appear above the paywall, use `Tranzmit.presentPlacementInRoute(context, ...)` for that placement. This includes terms and conditions popups, payment bottom sheets, error dialogs, snackbars, and pushed checkout screens. Both presentation APIs fade the paywall in by default; pass `transitionDuration` to customize the appearance timing for that placement. Otherwise, keep using the default `Tranzmit.of(context).presentPlacement(...)`.
