...
5. Tap **Present warmed "upgrade_pro"** and confirm the paywall appears from the warmed provider slot without a blank cold WebView moment.
...
The regular **Present "upgrade_pro"** button uses `Tranzmit.presentPlacementInRoute(...)` to demo Flutter UI above the paywall after CTA. Use that route path only when the customer app needs dialogs, bottom sheets, snackbars, or pushed checkout screens above the paywall.
